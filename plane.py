...
class Plane:
    ...
    def is_parallel_to(self, other: 'Plane', tolerance=1e-10):
    ...
    def __eq__(self, other: 'Plane'):
        ...
        # If the vector connecting the basepoints is orthogonal to the plane's normal vector,
        #  they must be the same plane
        base_vect = Vector([a-b for a,b in zip(self.basepoint, other.basepoint)])
        return self.normal_vector.is_orthogonal_to(base_vect)


    def intersection_with(self, other: 'Plane'):
        if self == other:
